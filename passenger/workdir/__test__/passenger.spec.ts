...
describe("Test for function expected value", () => {
  it("Returns evenly distributed values for boarded", () => {
    ...
    const locations = returnValue.boarded.map(
      (location: Record<string, any>) => location.location
    );
    ...
    const evenDistribution = locations.every(
      (location) => locationFreq[location] === 2
    );
